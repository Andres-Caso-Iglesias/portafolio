...
export default function TimelineDesktop({ items }: TimelineDesktopProps) {
  ...
  const getEventTheme = (item: TimelineItem) => {
    ...
    if (item.eduTitle) return { 
      border: "border-purple-500", 
      text: "text-purple-400", 
      bgSemi: "bg-purple-500/20", 
      bar: "bg-purple-500" 
    };
    if (item.expColor === "green") return { 
      border: "border-green-500", 
      text: "text-green-400", 
      bgSemi: "bg-green-500/20", 
      bar: "bg-green-500" 
    };
    if (item.expColor === "orange") return { 
      border: "border-amber-500", 
      text: "text-amber-400", 
      bgSemi: "bg-amber-500/20", 
      bar: "bg-amber-500" 
    };
    if (item.expColor === "blue") return { 
    ...
    <div className="relative min-h-[500px] hidden md:block w-full max-w-[98%] mx-auto mt-12 mb-20 px-4">
      ...
      {displayItems.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={cn("absolute -translate-y-1/2 -translate-x-1/2 flex flex-col items-center cursor-pointer group", topClass, zIndex)}
              ...
              >
                ...
                <div 
                  className={cn(
                    ...
                    isExp ? "border-t-8 border-t-white/10 -bottom-[8px]" : "border-b-8 border-b-white/10 -top-[8px]"
                  )} 
                />
