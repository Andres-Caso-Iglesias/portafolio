...
export function calculateTimelinePositions(items: TimelineItem[]): TimelineItem[] {
  ...
  const minDate = {
    year: minYear,
    month: Math.min(...allDates.filter(d => d.year === minYear).map(d => d.month)),
  };

  const maxYear = Math.max(...allDates.map(d => d.year));
  const maxDate = {
    year: maxYear,
    month: Math.max(...allDates.filter(d => d.year === maxYear).map(d => d.month)),
  };
  ...
  const positionedTimelineData = timelineData.map(item => {
    ...
    const startMonthsFromMin =
      (startDate.year - minDate.year) * 12 + (startDate.month - minDate.month);
