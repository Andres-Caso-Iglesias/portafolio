...
export const projects: Project[] = [
  {
    ...
    enDescription:
      'Passive web security auditing tool that analyzes HTTP response headers from any public URL, generates a security score (0-100), identifies missing or misconfigured headers, and maps results against OWASP Top 10 and NIS2 Directive. Includes TLS/SSL verification, DNS (SPF/DKIM/DMARC), security files (RFC 9116), SRI, tech fingerprinting with CVE database, and PDF/JSON export.',
  ...
  },
  {
    ...
    enChallenge:
      'The main challenge was designing a functional recruitment platform with two differentiated user roles (Candidate and Company) sharing the same auth base but with completely different profiles, dashboards, and functionalities. On the Flutter frontend, managing session state securely (SecureStorage + Provider), intelligent navigation based on authenticated role, and synchronization with a NestJS REST API. On the backend, implementing JWT authentication with global guards, user inheritance with TypeORM (1:1 relations), and DTOs with strict validation on every endpoint.',
    solution:
  ...
  },
  {
    name: 'FoodBites',
    description:
      'API RESTful para la gestión integral de food trucks: camiones, menús, pedidos, usuarios y ubicaciones. Backend Java con Spring Boot 3 y MySQL.',
    enName: 'FoodBites',
    enDescription:
      'RESTful API for comprehensive food truck management: trucks, menus, orders, users, and locations. Java backend with Spring Boot 3 and MySQL.',
    tech: [
      'Java 17',
      'Spring Boot 3.2',
      'MySQL',
  ...
  },
  {
    ...
    description:
      'Aplicación web para la gestión de huertos personales: organización de bancales, registro de cultivos e inventario de semillas. Frontend vanilla HTML/CSS/JS con Bootstrap, backend Spring Boot 3.2 + MySQL.',
    enName: 'Urban Garden Manager',
    enDescription:
      'Web application for personal garden management: plot organization, crop registration, and seed inventory. Vanilla HTML/CSS/JS frontend with Bootstrap, Spring Boot 3.2 + MySQL backend.',
    tech: [
      'Java 17',
      'Spring Boot 3.2',
      'MySQL',
  ...
  },
  {
    ...
    enChallenge:
      "Create a personal portfolio that not only presented projects but actively demonstrated the developer's technical capabilities through its own implementation. The challenge was to maintain clean code and strict typing while achieving an attractive and responsive user experience.",
    solution:
      'Arquitectura limpia con separación estricta de capas: datos (arrays tipados en /data), lógica (funciones puras en /lib), y presentación (componentes React en /components). Uso de TypeScript strict mode, interfaces explícitas para todos los tipos de datos, y componentes funcionales con hooks para el estado de UI.',
    enSolution:
      'Clean architecture with strict layer separation: data (typed arrays in /data), logic (pure functions in /lib), and presentation (React components in /components). Use of TypeScript strict mode, explicit interfaces for all data types, and functional components with hooks for UI state.',
    architecture:
      'Next.js 16 (App Router) + React 19 + TypeScript + Tailwind CSS v4. Arquitectura limpia con patrón container-presentational, gestión de estado con useState/useReducer, renderizado híbrido SSR + cliente.',
    enArchitecture:
      'Next.js 16 (App Router) + React 19 + TypeScript + Tailwind CSS v4. Clean architecture with container-presentational pattern, state management with useState/useReducer, hybrid SSR + client rendering.',
